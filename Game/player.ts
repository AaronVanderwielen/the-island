class Player {
    ...
    update(world: World, sound: Sound) {
        // player can perform action

        if (_.some(this.controls.actions, function (a) { return a.button === 3 && !a.recorded; })) {
            // player pressed 'y', toggle inventory
            this.inventory.toggle();
        }
        else if (this.inventory.focusedIndex != null) {
            // inventory is open
            if (_.some(this.controls.actions, function (a) { return a.button === 14 && !a.recorded; })) {
                // left on d-pad
            ...
            else if (_.some(this.controls.actions, function (a) { return a.button === 15 && !a.recorded; })) {
                // right on d-pad
