class World {
        ...
        if (objects.length > 0) {
            ...
            if (objects[0]) {
                ...
                if (dist < range) {
                    ...
                    if (invItem) {
                        // remove from map
                        objects[0].on.objects = _.reject(objects[0].on.objects, function (o: any) {
    ...
    createMapObjects() {
        for (var sy = 0; sy < this.sectionsY; sy++) {
            for (var sx = 0; sx < this.sectionsX; sx++) {
                var startX = sx * (this.numX / this.sectionsX),
    ...
    clientRender(tileset: HTMLImageElement, callback: Function) {
    ...
    renderBlockTerrain(block: Block, ySection: number, xSection: number, ctx: CanvasRenderingContext2D, tileset: HTMLImageElement) {
        var startBlockY = (block.y - (ySection * this.byps)) * this.tileSize,
            startBlockX = (block.x - (xSection * this.bxps)) * this.tileSize;

        ctx.drawImage(tileset, block.type * this.tileSize, 0, this.tileSize, this.tileSize, startBlockX, startBlockY, this.tileSize, this.tileSize);
    }
