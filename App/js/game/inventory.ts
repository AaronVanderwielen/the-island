...
export class Inventory {
    ...
    constructor(carryCapacity: number) {//, imgCache: Array<CachedImageData>) {
        this.carrying = [];
    ...
    pickup(mItem: MapObject.MapItem) {
        var numReceived = 0,
    ...
    putInBag(item: InventoryItem, stack: number) {
        ...
        var existingStack = _.find(this.carrying, function (i: InventoryItem) { return i && i.itemType == item.itemType && i.stack < i.maxStack; });
        if (existingStack) {
    ...
    dropInv(index: number, dropStack: boolean) {
        var dropped: MapObject.MapItem = null;
    ...
    //draw(canvas: HTMLCanvasElement, imgCache: Array<CachedImageData>) {
    ...
    //        if (this.holding) {
    //            var imgSet = Game.getCachedImgSet(this.holding.imgId, imgCache),
    //                img = imgSet.value[0][0];

    //            // draw item image
    //            tempCtx.drawImage(img, 0, 0, img.width, img.height, 0, 0, boxLength * 1.5, boxLength * 1.5);
    ...
    //                // draw item image
    //                tempCtx.drawImage(img, 0, 0, img.width, img.height, i * boxLength, boxLength * 2, boxLength, boxLength);
...
export class InventoryItem {
    ...
    static createFromMapItem(mItem: MapObject.MapItem) {
        var itemType = InventoryItem.getInventoryTypeFromMapType(mItem.itemType),
            newItem = new InventoryItem(itemType);
    ...
    static getInventoryTypeFromMapType(mapItemType: MapObject.ItemType) {
